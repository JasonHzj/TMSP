...
export default {
  ...
  methods: {
   ...
   async roder() {
      ...
      for (var o = 0; o < this.value4.length; o++) {
        this.timeSome.push(moment(this.value4[o]).format("YYYY-MM-DD"));
      }
      this.inputId = this.textarea1.split("\n");
      var listide;
      var listtitle;
      for (var c = 0; c < this.inputId.length; c++) {
        var tdInputid = this.inputId[c];
        //获取商品详情
      await this.$http
          .get("apis/cc/item/crowd/info.json?itemId=" + tdInputid)
          .then(res => {
            if (res.body.code === 0) {
                listide = res.body.data.detailUrl
                listtitle = res.body.data.title
         ...
         this.hcd(tdInputid,listide,listtitle)
  ...
  async hcd(tdInputid,listide,listtitle){
        
              for (var z = 0; z < this.timeSome.length; z++) {
          var someTime = this.timeSome[z];
              ...
              if (result.body.code === 0) {
                ...
                for (var i = 0; i < acee.length; i++) {
                  var dcd = acee[i];
                    ...
                    var jc = listtitle;
                    dcd.titleL = jc;
                    dcd.idL = this.$options.filters.stringL(jd);
                    dcd.timel = someTime
    ...
    handleDownload() {
                require.ensure([], () => {
                    ...
                    const tHeader = ['统计日期', '商品ID', '商品名称', 'SKUID', '加购件数'];
                    const filterVal = ['timel', 'idL', 'titleL', 'skuName', 'skuNewAddCartItemCnt'];
